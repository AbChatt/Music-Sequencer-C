...
BST_Node *newBST_Node(double freq, int bar, double index)
{
     ...
     * 
     * Piazza post said ignore this second comment
     ****/
    ...
    if (new_node != NULL)
    {
        ...
        {
            printf("Invalid index entered. Please enter an index between 0.0 and 1.0:");
            scanf("%lf", &index);
        }
...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
    ...
    if (root->key == key)
    {
        ...
        else
        {
            ...
            // copy data from successor to root
            root->bar = p->bar;
            root->freq = p->freq;
            root->index = p->index;
            root->key = p->key;
...
void BST_inOrder(BST_Node *root, int depth)
{
    ...
    if (p != NULL)
    {
        BST_inOrder(p->left, depth + 1);

        printf("Depth = %d, Bar:Index (%d:%f), F = %f Hz\n", depth, p->bar, p->index, p->freq);

        BST_inOrder(p->right, depth + 1);
    }
...
void BST_preOrder(BST_Node *root, int depth)
{
    ...
    if (p != NULL)
    {
        ...
        BST_preOrder(p->left, depth + 1);

        BST_preOrder(p->right, depth + 1);
    }
...
void BST_postOrder(BST_Node *root,int depth)
{
    ...
    if (p != NULL)
    {
        BST_postOrder(p->left, depth + 1);

        BST_postOrder(p->right, depth + 1);
...
void shift_notes(BST_Node *root, double old_freq, double new_freq)
{
    BST_Node *p = NULL;
    ...
    if (p != NULL)
    {
        ...
        shift_notes(root->left, old_freq, new_freq);

        shift_notes(root->right, old_freq, new_freq);
    }
}
...
void BST_shiftFreq(BST_Node *root, char note_src[5], char note_dst[5])
{
    ...
    {
        shift_notes(root, old_freq, new_freq);
    }
}
...
int binary_search(double note_freq[100], double query_freq)
{
    int i, j, k;
    ...
    while (i <= j)
    {
        ...
        if (note_freq[k] == query_freq)
        {
            return k;
        }
        else if (note_freq[k] < query_freq)
        {
            i = k + 1;
        }
        else
    ...
    if (i == 100 || note_freq[i] != query_freq)
    {
        return -1;
    }
    else
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
    ...
    if (root != NULL)
    {
        ...
        {
            i = binary_search(note_freq, root->freq);

            if (i > -1)
            {
                if (i + semitones >= 0 && i + semitones < 100)
                {
                    new_node = newBST_Node(note_freq[i + semitones], root->bar, root->index + time_shift);
